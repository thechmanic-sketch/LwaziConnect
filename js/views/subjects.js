function rSubjects(area,clsName){
 ...
 <div id="sm1"><div class="card">
  ...
  ${D.students.filter(s=>s.cls===cls).map(s=>{const ms=SUBS.map(()=>Math.max(30,Math.min(100,Math.floor(s.avg+(Math.random()*22-11)))));const avg=Math.round(ms.reduce((a,b)=>a+b,0)/ms.length);const g=avg>=80?'A':avg>=70?'B':avg>=60?'C':avg>=50?'D':'F';return`<tr><td><div class="flex ic g6"><div class="av av-s" style="background:${s.bg};color:${s.fg}">${s.ini}</div>${s.name}</div></td>${ms.map(m=>`<td style="font-weight:500;color:${m<50?'var(--r)':m<60?'var(--ad)':'var(--sm)'}">${m}</td>`).join('')}<td style="font-weight:700">${avg}%</td><td><span class="pill ${gc(g)}">${g}</span></td></tr>`;}).join('')}
  </tbody></table></div>
 </div></div>
