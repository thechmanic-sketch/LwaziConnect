function rSubjects(area,clsName){
 ...
 <div id="sm1"><div class="card">
  ...
  ${D.students.filter(s=>s.cls===cls).map(s=>{
   const ms=SUBS.map(sub=>{const rec=(s.marks||[]).find(m=>m.subject===sub&&m.term==='Term 3');return rec?Math.round(Number(rec.mark)/Number(rec.out_of)*100):null;});
   const known=ms.filter(m=>m!==null);
   const avg=known.length?Math.round(known.reduce((a,b)=>a+b,0)/known.length):0;
   const g=avg>=80?'A':avg>=70?'B':avg>=60?'C':avg>=50?'D':avg>0?'F':'—';
   return`<tr><td><div class="flex ic g6"><div class="av av-s" style="background:${s.bg};color:${s.fg}">${s.ini}</div>${s.name}</div></td>${ms.map(m=>`<td style="font-weight:500;color:${m===null?'var(--sl)':m<50?'var(--r)':m<60?'var(--ad)':'var(--sm)'}">${m===null?'—':m}</td>`).join('')}<td style="font-weight:700">${known.length?avg+'%':'—'}</td><td><span class="pill ${gc(g)}">${g}</span></td></tr>`;}).join('')}
  </tbody></table></div>
 </div></div>
