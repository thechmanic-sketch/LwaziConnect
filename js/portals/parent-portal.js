...
function rParentPortal(){
 ...
 <div style="background:var(--bg);padding:16px 0">
  ...
  <div class="card"><div class="card-head"><div class="card-title"><i class="ti ti-message-circle"></i>Messages</div><button class="btn btn-g" onclick="V('messages')"><i class="ti ti-send" style="font-size:11px"></i>Open Messages</button></div>
  ${D.messages.slice(0,3).map(m=>`<div style="display:flex;align-items:flex-start;gap:9px;padding:9px 0;border-bottom:1px solid var(--sp);cursor:pointer" onclick="V('messages')"><div class="av av-s" style="background:${m.bg};color:${m.fg}">${m.ini}</div><div style="flex:1"><div style="font-size:12px;font-weight:600">${m.from}</div><div style="font-size:11px;color:var(--sl);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:240px">${m.thread.length?m.thread[m.thread.length-1].text:''}</div></div><div class="tsm">${m.time}</div></div>`).join('')||'<div class="tsm" style="padding:9px 0">No messages yet.</div>'}
  </div>
 </div>`;
